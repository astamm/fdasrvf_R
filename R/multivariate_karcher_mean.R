...
multivariate_karcher_mean <- function(
  beta,
  mode = "O",
  alignment = TRUE,
  rotation = FALSE,
  ...
  while (itr < maxit) {
    if (verbose) cli::cli_alert_info("Iteration {itr}/{maxit}...")

    if (mode == "O" || !scale) basis <- NULL else
  ...
  for (n in 1:N) {
    scl <- 1
    if (scale) scl <- qmean_norm / len_q[n]
