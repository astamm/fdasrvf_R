...
pointwise_karcher_mean <- function(
  q,
  qmean,
  basis = NULL,
  scale = FALSE,
  ...
  for (n in 1:N) {
    ...
    if (ms == "median") {
      ...
      if (d_i[n] > 0) v_d[,, n] <- v[,, n] / d_i[n] else v_d[,, n] <- v[,, n] # normalize v_i
    } else {
  ...
  if (ms == "median") {
    ...
    sum_dinv <- sum(1 / d_i[d_i > .Machine$double.eps^0.5]) # avoid division by 0
    vbar <- sumv / sum_dinv
  } else {
    # run for mean only
